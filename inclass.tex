...
\markright{Ryan Ordille\hfill COMP 302: Programming Languages and Paradigms\hfill}

These course notes are for COMP 302: Programming Languages and Paradigms, offered at McGill University in the Winter 2012 semester, taught by Professor Brigitte Pientka. These notes are simply what the instructor wrote on the board, and may contain errors. The notes are written by me, Ryan Ordille, and no copyright infringement is intended.  

These course notes can be found at \url{https://github.com/ryanordille/c302w12}.
...
If-statements must have a boolean as the guard, and two possible branches must be \emph{of the same type}. However, the type checker is not ``smart".
...
\textbf{Binding:} variable name paired with a value. Note that bindings are different than assignments found in imperative languages. 
...
\begin{verbatim}
    ...
    add' (x : int, y : int) = x + y
\end{verbatim}
The function \verb~add'~ takes in one argument -- a tuple with two elements -- whereas \verb~add~ takes in two separate arguments.
...
A list is either empty (nil) or, if $A$ is an element and $L$ is a list, \verb!Cons(A,L)!.
...
A binary tree is either empty or, if L and R are binary trees and v is a value of type $\alpha$, \verb!Node(v,L,R)!.
...
There's a ``best of both worlds" strategy we can also use -- the \emph{call-by-need} strategy. In our original example, the call-by-need strategy will bind \verb~x~ to the expression \verb~e1~, then evaluate \verb~e2~, but memorize the result of evaluating \verb~e1~.
...
val x = Susp (fun () => 
  case memo of 
    None =>
        let val y = horribleComp(522) in memo := y+y end
    | Some y => y
...
\begin{itemize}
    ...
    \item What exactly is an expression? \\
            Code \textrightarrow Parser (syntax checker) \textrightarrow Type Checker (static semantics) \textrightarrow Interpreter (operational semantics)
\end{itemize}
...
\[
    \infer{\Gamma \vdash \verb~fn x => if x=0 then 4 else x+2 : int~}{
        \Gamma, \verb~x:int ~ \vdash \verb~if x=0 then 4 else x+2 : int~
    }
\]

The following rule cannot be interpreted as inferring a type:
\[
    \infer{\Gamma \vdash \verb~fn x => e : ~ T_1 \rightarrow T_2}{
...
What is the most general type of an expression? $\alpha$ -- the ``principle type".

Extremely important rule:
\[
    \infer{\Gamma \vdash e_1 e_2 : T_2}{
...
\[
    \infer{\Gamma \vdash \verb~!e : T~}{
        \Gamma \vdash \verb~ e : T ref~
    }
\]
\[
    \infer{\Gamma \vdash \verb~ ref e : ref T~}{
        \Gamma \vdash e : T
    }
\]
\[
    \infer{\Gamma \vdash \verb~e := e' : unit~}{
        \Gamma \vdash \verb~ e : T ref~
        &
        \Gamma \vdash e^{\prime} : T
    }
\]
...
\begin{enumerate}
    \item Are \emph{all} substitution instances of \verb~e~ well-typed? \\
        If e has some type T and constrains some type variables $\alpha_1, \ldots, \alpha_n$, then e has type \verb~[T1/'a1 ... Tn/'an] T~ for every $T_1, \ldots T_n$?
...
----------------------------------------------------
G |- if e then e1 else e2 => T2 / T=bool ^ T1=t2 ^ C ^ C1 ^ C2
...
\[
    \infer{\Gamma \vdash n \Rightarrow \verb~ int / tt~}{}
\]
\[
    \infer{\Gamma \vdash \verb~ true => bool / tt~}{}
\]
\[
    \infer{\Gamma \vdash \verb~ x => T / tt~}{
        \Gamma (x) = T
    }
\]
...
\[
    \infer{\Gamma \vdash \verb~ let x=e1 in e2 : T~}{
        \Gamma \vdash [e_1 / x] e_2 : T
    }
\]
...
\[
    \infer{\Gamma \vdash \verb~ let x=e1 in e2 : T~}{
        \Gamma \vdash e_1 : S
        &
        \Gamma_1, x, \verb~generalize(S)~ \vdash e_2 : T
    }
\]
...
Example:
\[
    \infer[\text{by conversion}]{\vdash 3 \Leftarrow \verb~int~}{
...
\[
    \infer[\text{covariant}]{T_1 \times T_2 \leq S_1 \times S_2}{
        T_1 \leq S_1
        &
        T_2 \leq S_2
    }
...
\[
    \infer[\text{depth subtyping}]{\{x_1 : T_1, \dots, x_n : T_1 \} \leq \{ x_1 : S_1, \dots, x_n : S_1 \} }{
...
\[
    \infer[\text{Upcast}]{\Gamma \vdash (T) e : T}{
        \Gamma \vdash e : S
        &
        S \leq T
     }
